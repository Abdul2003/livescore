...
import HomeLayout from './England/components/header'
...
function Livescore() {
  ...
  useEffect(() => {
    ...
    fetch(
      'https://api-football-v1.p.rapidapi.com/v3/fixtures?live=all&league=39&season=2021',
      options
    )
      ...
      <>
        ...
        <List
          ...
          renderItem={(item) => (
            <Card className="card">
              <Link
                to={`/England/statistics?fixture=${item.fixture.id}`}
                className="cardContent"
              >
                <p className="status">{item.fixture.status.long}</p>
                <p className="time">{item.fixture.status.elapsed}</p>
